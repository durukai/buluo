...
//通过栏目ID获取对应的栏目名称/别名等
function getlisturl($cid,$type='list_url'){
    $arr=list_search(F('_ppvod/list'),'list_id='.$cid);
	if(is_array($arr)){
		return $arr[0][$type];
	}else{
...
function getlisttitle($cid,$type='list_title'){
    $arr=list_search(F('_ppvod/list'),'list_id='.$cid);
	if(is_array($arr)){
		return $arr[0][$type];
	}else{
...
function getlistkeywords($cid,$type='list_keywords'){
    $arr=list_search(F('_ppvod/list'),'list_id='.$cid);
	if(is_array($arr)){
		return $arr[0][$type];
	}else{
...
function getlistdescription($cid,$type='list_description'){
    $arr=list_search(F('_ppvod/list'),'list_id='.$cid);
	if(is_array($arr)){
		return $arr[0][$type];
	}else{
...
//rewrite重写
	if(C('url_rewrite')){
		...
		if($model == 'Home-vod/show'){
			$reurl = str_replace(array('$id','$mcid','$page','$order','$picm'),array($params['id'],$params['mcid'],$params['p'],$params['order'],$params['picm']),C('rewrite_vodlist'));
			...
			$reurl = str_replace(array('$id','$mcid','$page','$wd','$area','$language','$actor','$director','$year','$letter','$order'),array($params['id'],$params['mcid'],$params['p'],$params['wd'],$params['area'],$params['language'],$params['actor'],$params['director'],$params['year'],$params['letter'],$params['order']),C('rewrite_vodtype'));
...
//字母封面
//function ff_list_letter_url_dir($sid,$cid){
//	if('vod' == $sid){
//		$listdir = str_replace_dir(C('url_vodlist_letter'),$id,$cid,$name);
//	}
//	return $listdir;
//}
//字母封面
...
function ff_data_url_dir($sid,$id,$cid,$name,$page){
	//专题直接返回
	if('special' == $sid){
		return trim(C('url_special').ff_pinyin($name));
	}
	//影视或文章
	if('vod' == $sid){
...
//}
//$list_dir = getlistname($cid,'list_dir'); 
//$playurl = '/'.$list_dir.'/'.$id.'/'.$sid.'-'.$pid.'.html'; 
//return $playurl;
//}
function ff_play_url($id,$sid,$pid,$cid,$name){
	//静态模式
...
function ff_tag_url($str,$sid=1){
	if($sid==2){
		return UU('Home-tag/news',array('wd'=>urlencode($str)),false,true);
	}else{
		return UU('Home-tag/vod',array('wd'=>urlencode($str)),false,true);
	}
}
// 内容页Tag链接
...
function ff_mcat_url($str,$cid)
{
	...
	$html = '';
	foreach($mcids as $v){
		$url = UU('Home-vod/show', array('id'=>$cid, 'mcid'=>$v),false,true);
		$name = M('Mcat')->where("m_cid = {$v}")->find();
		$html .= "<a href='".$url."' target='_blank'>{$name['m_name']}</a>";
	}
	return $html;
}
...
function ff_mcat_admin($str,$cid)
{
	if(empty($str)){
		return "无分类";
	}
...
// 获取搜索
function ff_director_url($str,$type="director",$sidname='vod',$action='search'){
	$array = array();
    $str = str_replace(array('/','|',',','，'),' ',$str);
	$arr = explode(' ',$str);
	foreach($arr as $key=>$val){
...
function ff_mysql_vod($tag){
	...
	//止步：
	if ($tag['mcid']) {
		$where["FIND_IN_SET({$tag['mcid']},vod_mcid)"] = array("gt","0");
	}
	if ($tag['weekday']) {
		$where["FIND_IN_SET({$tag['weekday']},vod_weekday)"] = array("gt","0");
	}
	if ($tag['prty']) {
		$where["FIND_IN_SET({$tag['prty']},vod_prty)"] = array("gt","0");
	}
    if ($tag['diantai']) {
		$where['vod_diantai'] = array('eq',''.$tag['diantai'].'');
	}
	//止步：
	if ($tag['day']) {
	...
	if ($tag['zhanti']) {
		$hits = explode(',',$tag['zhanti']);
		if (count($hits) > 1) {
			$where['vod_zhanti'] = array('between',$hits[0].','.$hits[1]);
		}else{
			$where['vod_zhanti'] = array('gt',$hits[0]);
		}
	}
	if ($tag['beijin']) {
		$hits = explode(',',$tag['beijin']);
		if (count($hits) > 1) {
			$where['vod_beijin'] = array('between',$hits[0].','.$hits[1]);
		}else{
			$where['vod_beijin'] = array('gt',$hits[0]);
		}
	}
	if ($tag['up']) {
		$up = explode(',',$tag['up']);
	...
	if($tag['page']){
		...
		$list[0]['page'] = $pages;
		$list[0]['pagetop'] = $pagestop; //止步：
		$list[0]['pagecount'] = $pagescount; //止步：
		$list[0]['pags'] = $pags; //止步：					
	}else{
	...
	foreach($list as $key=>$val){
		...
		$list[$key]['vod_miaopicurl'] = ff_img_url($list[$key]['vod_miaopic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_miaosuopicurl'] = ff_img_url($list[$key]['vod_miaosuopic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_zhuantipicurl'] = ff_img_url($list[$key]['vod_zhuantipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_sipicurl'] = ff_img_url($list[$key]['vod_sipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_siipicurl'] = ff_img_url($list[$key]['vod_siipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_siiipicurl'] = ff_img_url($list[$key]['vod_siiipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_siiiipicurl'] = ff_img_url($list[$key]['vod_siiiipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_siiiiipicurl'] = ff_img_url($list[$key]['vod_siiiiipic'],$list[$key]['vod_content']);	//海报 
		$list[$key]['vod_picurl_small'] = ff_img_url_small($list[$key]['vod_pic'],$list[$key]['vod_content']);
		//以下为增加最后一集调用
        $lastplayurl = ff_play_url_end(trim($list[$key]['vod_url']));
        $list[$key]['lastplay_name'] = $lastplayurl[2];
        $list[$key]['lastplay_url'] = ff_play_url($list[$key]['vod_id'],$lastplayurl[0],$lastplayurl[1],$list[$key]['vod_cid'],$list[$key]['vod_name']);
	}
	//是否写入数据缓存
	if(C('data_cache_vodforeach') && C('currentpage') < 2 ){
 ...
 * 正在热映
 */
function home_hot_play() {
	$model = M('Vod');
	$where = array(
		'vod_cid' => array('in', movie_cids()),
		'vod_filmtime' => array('elt', time())
	);
	$order = 'vod_filmtime DESC';
	$list = $model->field('vod_id,vod_name,vod_filmtime,vod_cid,vod_pic,vod_jumpurl')->where($where)->order($order)->limit(30)->select();
	if($list) {
		foreach ($list as $key => $val) {
			$list[$key]['vod_readurl'] = ff_data_url('vod',$val['vod_id'],$val['vod_cid'],$val['vod_name'],1,$val['vod_jumpurl']);
		}
	}
	return $list;
}
 ...
 * 即将上映
 */
 function home_after_playtv() {
	$model = M('Vod');
	$where = array(
		'vod_cid' => array('in', tv_cids()),
		'vod_filmtime' => array('gt', time())
	);
	$order = 'vod_filmtime ASC';
	$list = $model->field('vod_id,vod_filmtime,vod_name,vod_cid,vod_pic,vod_jumpurl')->where($where)->order($order)->limit(30)->select();
	if($list) {
		foreach ($list as $key => $val) {
			$list[$key]['vod_readurl'] = ff_data_url('vod',$val['vod_id'],$val['vod_cid'],$val['vod_name'],1,$val['vod_jumpurl']);
		}
...
function tv_cids(){
	$data = M('List')->field('list_id')->where(array('list_pid' => 2))->select();
	$cids = array();
	foreach ($data as $val) {
		$cids[] = $val['list_id'];
	}
...
function movie_cids(){
	$data = M('List')->field('list_id')->where(array('list_pid' => 1))->select();
	$cids = array();
	foreach ($data as $val) {
		$cids[] = $val['list_id'];
	}
...
function SendMail($address,$title,$message,$mailSetting,$sender='')
{
	...
	$mail->From=$mailSetting["MAIL_ADDRESS"];  // 设置发件人名字
	$mail->FromName=$sender;  // 设置邮件标题
	$mail->Subject=$title;          // 设置SMTP服务器。
	$mail->Host=$mailSetting['MAIL_SMTP'];     // 设置为"需要验证" ThinkPHP 的C方法读取配置文件
	$mail->SMTPAuth=true;           // 设置用户名和密码。
	$mail->Username=$mailSetting['MAIL_LOGINNAME'];
	$mail->Port=$mailSetting['MAIL_PORT'];
	$mail->IsHTML(true);
	$mail->Password=$mailSetting['MAIL_PASSWORD']; // 发送邮件。
	if($mail->Send()){
		return true ;
	}else{
...
function quickSendMail($address,$title,$message){
	...
	if($mailsetting){
		$setting = array(
				"MAIL_ADDRESS" => $mailsetting["mail_from"],
				"MAIL_SMTP" => $mailsetting["mail_server"],
				"MAIL_LOGINNAME" => $mailsetting["mail_user"],
...
function getAppPath(){
	return C('site_url').substr(__ROOT__,1);
}
function getAppPatht(){
if(C('url_rewrite')){
$useurl= C('site_path');
}
else{
...
function getavatar(){
	return 'ffcms/userdir/000/00/00/';
}
/**
 * 获取当前页面完整URL地址
 */
...
/**
 * 获取用户昵称
 * 不传入userid取当前用户username
 * 传入field，取用户$field字段信息
 */
function get_nickname($userid='', $field='') {
	$return = '';
	if(is_numeric($userid)) {
		$member_db = D("User.User");
		$memberinfo = $member_db->getMember(array('userid'=>$userid));
		if(!empty($field) &&  isset($memberinfo[$field]) &&!empty($memberinfo[$field])) {
			$return = $memberinfo[$field];
		} else {
			$return = empty($memberinfo['nickname']) ? $memberinfo['username'] : $list['nickname'];
		}
	} else {
		$return .= cookie_decode(cookie('_username'));
	}
	return $return;
}
/**
...
function ps_getavatar($uid) {
	...
	//判断 是否有图像
	$member_db = D("User.User");
	$filename = './ffcms/userdir/000/00/00/'.$uid.'_avatar_middle.jpg';
	if (is_file($filename)){
		$appPath = getAppPath(); //noavatar_middle.gif  noavatar_small
		$appatar = getavatar();
	    $dir1 = ceil($uid / 10000);
	    $dir2 = ceil($uid % 10000 / 1000);
	    $url = $appPath.$appatar.''.$uid.'_';
	    $avatar = array('200'=>$url.'avatar_big.jpg', 
			            '120'=>$url.'avatar_middle.jpg', 
			            '48'=>$url.'avatar_small.jpg', 
					    '30'=>$url.'avatar_small.jpg');		 
		return $avatar ;
	}
	    $appPath = getAppPath();
		$avatar = array('200'=>$appPath.'Public/user/images/noavatar_big.gif',
				        '120'=>$appPath.'Public/user/images/noavatar_middle.gif',
				        '48'=>$appPath.'Public/user/images/noavatar_small.gif',
				        '30'=>$appPath.'Public/user/images/noavatar_small.gif',);
	return $avatar;
}
function getAvatarBySize($size='30'){
	$avatar = ps_getavatar();
	if(is_array($avatar)){
		return 	$avatar[$size] ;
	}
}
function ps_getavatarImg($uid,$size='30'){
	$ps_getavatar = ps_getavatar($uid);
	if(!empty($ps_getavatar)){
		return $ps_getavatar[$size] ;
	}
}
...
function getcmvodname($str)
{
	$html = '';
	$name = M('Vod')->where("vod_id = {$str}")->find();
	$html .= " {$name['vod_name']}";
	return $html;
}
?>