...
class CommentModel extends Model {
	protected $_validate=array(
		...
		array('comment_id','number','您没有指定评论ID！',1),
		array('comment_sid','require','您没有指定评论模型！',1),
	...
	protected $_auto=array(
		...
		array('comment_sid','intval',1,'function'),
		array('ip','get_client_ip',1,'function'),
		array('creat_at','time',3,'function'),
	);
	//检测验证码是否正确
	...
	public function check_reinput(){
		$cookie = $_COOKIE['comment_'.intval($_POST['comment_sid']).'_'.intval($_POST['comment_cid'])];
		if(isset($cookie)){
			return false;
		}
	}
