...
class XmlnapianzyAction extends BaseAction{
    ...
    public function xml_feifei($array_url){
		//组合资源库URL地址并获取XML资源
		$http_url = str_replace('@', '-', admin_ff_url_repalce($array_url['xmlurl']).'index.php?s=plus-vip-json-action-'.$array_url['action'].'-vodids-'.$array_url['vodids'].'-cid-'.$array_url['cid'].'-play-'.$array_url['play'].'-inputer-'.$array_url['inputer'].'-wd-'.urlencode($array_url['wd']).'-h-'.$array_url['h'].'-p-'.$array_url['page']);
		 $json = ff_file_get_contents($http_url);
		//print_r($http_url);
		//$json = ff_file_get_contents($http_url,30);
		if ($json) {
			...
			$array_tpl = array();
			$array_url['p'] = '{!page!}';
			$array_tpl['pageurl'] = urldecode(U('Admin-Xmlnapianzy/Feifei',$array_url));
			...
			foreach($json->data as $key=>$value){
				...
				$array_vod[$key]['vod_diantai'] = $value->vod_diantai;
				$array_vod[$key]['vod_tvcont'] = $value->vod_tvcont;
				$array_vod[$key]['vod_prty'] = $value->vod_prty;
    ...
    public function xml_caijia($array_url){
		...
		if ($xml) {
			...
			$array_url['p'] = '{!page!}';
			$array_tpl['pageurl'] = urldecode(U('Admin-Xmlnapianzy/Caijia',$array_url));
			...
			foreach($vod_array[1] as $key=>$value){
				...
				$listvod[$key]['vod_letters'] = $this->getRandChar(20);
				//以下参数只在内容页才有
