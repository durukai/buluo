<?php
class XmljiutousheAction extends BaseAction{
	// 资源列表
    ...
    public function xml_feifei($array_url){
		...
		if ($json) {
			...
			$array_tpl = array();
			$array_url['p'] = '{!page!}';
			$array_tpl['pageurl'] = U('Admin-Xmljiutoushe/Feifei',$array_url);
    ...
    public function xml_caijia($array_url){
		...
		if ($xml) {
			...
			$array_url['p'] = '{!page!}';
			$array_tpl['pageurl'] = U('Admin-Xmljiutoushe/Caijia',$array_url);
			...
			foreach($vod_array[1] as $key=>$value){
				...
				$listvod[$key]['vod_keywords'] = $vod_array[17][$key].",".$vod_array[16][$key]; //关联词
				$listvod[$key]['vod_gold'] = intval($vod_array[18][$key]); //评分值
				$listvod[$key]['vod_golder'] = intval($vod_array[19][$key]); //评分人数
				$listvod[$key]['vod_stars'] = intval($vod_array[20][$key]); //推荐
				if($vod_array[20][$key]>0){$listvod[$key]['vod_stars']=5;}//指定5星
				...
				if($array_url['action']){
					...
					$vod_array[14][$key] = preg_replace('/<dd flag="([\s\S]*?)"><\!\[CDATA\[([\s\S]*?)\]\]><\/dd>/', '$1\$\$\2', $vod_array[14][$key]);
                                        $url_arr = explode("$$$", $vod_array[14][$key]);
                                        $vod_plays = array();
                                        $vod_urls = array();
                                        foreach($url_arr as $url_item){
                                                $_url_item = explode("$$", $url_item);
                                                $vod_plays[] = $_url_item[0];
                                                $vod_urls[] = preg_replace("/\\$[^\\$]*?#/", "\n", preg_replace("/\\$[^\\$]*$/", "", $_url_item[1]));
                                        }
                                        $listvod[$key]['vod_play'] = str_replace(array('乐视','奇艺','土豆','qq播客','搜狐','百度影音','youku','56','优酷','迅播高清','SWF数据'),array('letv','qiyi','tudou','qq','sohu','bdhd','yuku','wole','yuku','gvod','swf'),implode('$$$',$vod_plays));
					$listvod[$key]['vod_url'] =  htmlspecialchars_decode(implode('$$$',$vod_urls));
				}else{
