<?php
class ffcmsAction extends BaseAction{
  //下载远程图片
  public function picdown(){
    $IsPic = $_GET['IsPic'];
    if("1" == $IsPic){
      die("设置为不下载图片！<span style='display:none;'>图片恭喜您,所有远程图片已经下载完成！</span>");
    }
	  C('upload_http',1);
	  $img = D('Img');
	  $rs = D("Vod");
	  ...
	  if($list){
		  ...
		  echo'请稍等一会，正在释放服务器资源...<meta http-equiv="refresh" content='.C('play_collect_time').';url=?s=Admin-ffcms-PicDown&IsPic='.$IsPic.'>';
		  echo'</div>';
	  }else{
		  $nData = $_GET['nData'];
		  if("1" == $nData)
		  	echo"恭喜您,所有远程图片已经下载完成！<span style='display:none;'>图片恭喜您,所有远程图片已经下载完成！</span>";
		  else
      	  	echo'请稍等一会，正在释放服务器资源...<meta http-equiv="refresh" content=1;url=?s=Admin-ffcms-PicDown&IsPic='.$IsPic.'&nData=1>';
	  }
  }
  
  public function makehtml(){
    if(false == C('url_html')){
      die("\"视频内容页\"模块动态运行，不需要生成静态网页！<span style='display:none;'>一键生成全部搞定</span>");
    }
    $IsMovie = $_GET['IsMovie'];
    if("1" == $IsMovie){
      die("设置为不生成静态页面！<span style='display:none;'>一键生成全部搞定</span>");
    }
    $Ttime=$_REQUEST['Ttime'];
    $ppCount=$_REQUEST['ppCount'];
    
	  $rs = D("Vod");
    $where['vod_status'] = array('eq',1);
	  $where['vod_cid'] = array('gt',0);
    $where['vod_addtime'] = array('gt',strtotime($Ttime));
	  ...
	  if (empty($array )) {
      die($Ttime."之后没有数据更新,暂不需要生成！<span style='display:none;'>一键生成全部搞定</span>");
	  }
	  ...
	  echo'</ul>';
	  if ($page < $totalpages) {
		  $jumpurl = '?s=Admin-ffcms-MakeHtml&IsMovie='.$IsMovie.'&ppCount='.$ppCount.'&Ttime='.$Ttime.'&page='.($page+1);
	  }else{
		  ...
		  $ids = implode(',',$list[1]);
		  $jumpurl = '?s=Admin-ffcms-Vodlist-jump-1-ids_list_1-'.$ids;
	  }
	  if ($page < $totalpages) {
  ...
  public function newslist(){
	...
	public function list_cache_array($sid=1){
		...
		$this->check(C('url_html_list'),getsidname($sid).'列表页','?s=Admin-ffcms-Index-jump-'.$jump);
  ...
  public function list_create_array($sid){
		...
		$jump = intval($_REQUEST['jump']);
		if($sid==1){
			$nextcreate = '?s=Admin-ffcms-Vodlist_create-jump-'.$jump.'-key-'.$key;
			$sidname = '视频';
		}else{
			$nextcreate = '?s=Admin-ffcms-Newslist_create-jump-'.$jump.'-key-'.$key;
			$sidname = '资讯';
		}
		//断点生成(写入缓存)
		...
		F('_create/list',NULL);
		if($jump){
			$this->jump('?s=Admin-ffcms-Index-jump-'.$jump,'列表页已经全部生成，下次将生成网站首页。');
		}
		F('_create/nextcreate',NULL);
		$this->jump('?s=Admin-ffcms-Show','恭喜您，列表页已经全部生成！');
	}
	//读取资讯内容按分类
	public function newsclass(){
		...
		$page = !empty($_GET['page']) ? intval($_GET['page']) : 1;
		//检测是否需要生成
		$this->check(C('url_html'),'资讯内容页','?s=Admin-ffcms-Newslist-ids_2-'.$ids.'-jump-'.$jump);
		//断点生成(写入缓存)
		F('_create/nextcreate','?s=Admin-ffcms-Vodclass-ids_2-'.$ids.'-jump-'.$jump.'-page-'.$page);
		//查询数据开始	
		$rs = D("News");
		...
		if (empty($array)) {
			$this->assign("jumpUrl",'?s=Admin-ffcms-Show');
		...
		echo'</ul>';
		//组合回跳URL路径与执行跳转
		$jumpurl = '?s=Admin-ffcms-Show';
		if($page < $totalpages){
			$jumpurl = '?s=Admin-ffcms-Newsclass-ids_2-'.$ids.'-jump-'.$jump.'-page-'.($page+1);
		}else{
			if($jump){
				$jumpurl = '?s=Admin-ffcms-Newslist-ids_list_2-'.$ids.'-jump-'.$jump;
			}
		}
		if($page < $totalpages){
	...
	public function newsday(){
		...
		$jumpurl = '?s=Admin-ffcms-Show';
		if ($page < $totalpages) {
			$jumpurl = '?s=Admin-ffcms-Newsday-jump-'.$jump.'-mday_2-'.$mday.'-min-'.$mid.'-page-'.($page+1);
		}else{
			if($jump){
				$listarr = F('_ppvod/list');
				foreach($listarr as $key=>$value){
				...
				$ids = implode(',',$list[1]);
				$jumpurl = '?s=Admin-ffcms-Newslist-jump-1-ids_list_2-'.$ids;			
			}
		}
		if ($page < $totalpages) {
			$this->jump($jumpurl,'第('.$page.')页已经生成完毕，正在准备下一个。');
  ...
  public function news_read_create($array){
	...
	public function vodclass(){
		...
		$page = !empty($_GET['page']) ? intval($_GET['page']) : 1;
		$this->check(C('url_html'),'视频内容页','?s=Admin-ffcms-Vodlist-ids_1-'.$ids.'-jump-'.$jump);
		//断点生成(写入缓存)
		F('_create/nextcreate','?s=Admin-ffcms-Vodclass-ids_1-'.$ids.'-jump-'.$jump.'-page-'.$page);	
		$rs = D("Vod");
		$where['vod_status'] = array('eq',1);
		...
		$jumpurl = '?s=Admin-ffcms-Show';
		if($page < $totalpages){
			$jumpurl = '?s=Admin-ffcms-Vodclass-ids_1-'.$ids.'-jump-'.$jump.'-page-'.($page+1);
		}else{
			if($jump){
				$jumpurl = '?s=Admin-ffcms-Vodlist-ids_list_1-'.$ids.'-jump-'.$jump;
			}
		}
		if($page < $totalpages){
	...
	public function vodday(){
		...
		echo'</ul>';
		if ($page < $totalpages) {
			$jumpurl = '?s=Admin-ffcms-Vodday-jump-'.$jump.'-mday_1-'.$mday.'-min-'.$mid.'-page-'.($page+1);
		}else{
			if($jump){
				$listarr = F('_ppvod/list');
				foreach($listarr as $key=>$value){
  ...
  public function baidu($id){
		...
		if (empty($id)) {
			...
			$this->assign("jumpUrl",'?s=Admin-ffcms-Google-jump-'.$jump);
  ...
  public function google($id){
		...
		if (empty($id)) {
			$this->assign("waitSecond",2);
			$this->assign("jumpUrl",'?s=Admin-ffcms-Rss-jump-'.$jump);
  ...
  public function rss($id){
		...
		if (empty($id)) {
			die("一键生成全部搞定");
//			$this->assign("jumpUrl",'?s=Admin-ffcms-Maps-jump-'.$jump);
//        	$this->success('Rss订阅文件生成成功！');
  ...
  public function index(){
		...
		if ($jump) {
			//$this->assign("jumpUrl",'?s=Admin-ffcms-Mytpl-jump-'.$jump);
  ...
  public function mytpl(){
		...
		foreach($list_dir as $key=>$value){
			if(ereg("my_(.*)\.html",$value['filename'])){
				C('html_file_suffix',$suffix);
				if("new" == str_replace(array('my_','.html'),'',$value['filename'])){
      ...
      $this->assign("jumpUrl",'?s=Admin-ffcms-Baidu-jump-'.$jump);
			$this->success('自定义模板生成完毕，准备生成百度地图！');
		}else{
			$this->assign("jumpUrl",'?s=Admin-ffcms-Show');
  ...
  public function speciallist(){
		//检测是否需要生成
		$this->check(C('url_html'),'专题列表页','?s=Admin-ffcms-Show');
		//查询数据开始	
		$page = !empty($_GET['page']) ? intval($_GET['page']) : 1;
		...
		echo'</ul>';
		exit();
		$this->jump('?s=Admin-ffcms-Show','恭喜您，专题列表页已经全部生成！');
	}
	//生成专题内容分页处理
  public function specialclass(){
		//检测是否需要生成
		$this->check(C('url_html'),'专题内容页','?s=Admin-ffcms-Show');
		//查询数据开始	
		$page = !empty($_GET['page']) ? intval($_GET['page']) : 1;
		$rs = D("Special");
		$where['special_status'] = array('eq',1);
		...
		if (empty($array)) {
			$this->assign("jumpUrl",'?s=Admin-ffcms-Show');$this->error('没有数据，不需要生成！');
		}
		echo'<ul id="show" style="font-size:12px;list-style:none;margin:0px;padding:0px;font-family:宋体">';
		...
		$jumpurl = '?s=Admin-ffcms-Show';
		if($page < $totalpages){
			$jumpurl = '?s=Admin-ffcms-Specialclass-page-'.($page+1);
  ...
  public function show(){
	...
	//判断生成条件
  public function check($html_status,$html_err,$jumpurl='?s=Admin-ffcms-Show'){	
	    if (!$html_status) {
