<?php
class PiccAction extends BaseAction{
	//下载远程图片
    public function down(){
		...
		if('fail'==trim($_GET['id'])){
			$rs->execute('update '.C('db_prefix').'vod set vod_miaopic=REPLACE(vod_miaopic,"httpf://", "http://")');
		}
		$count = $rs->where('Left(vod_miaopic,7)="http://"')->count('vod_id');
		$list = $rs->where('Left(vod_miaopic,7)="http://"')->order('vod_addtime desc')->limit(C('upload_http_down'))->select();
		if($list){
			...
			foreach($list as $key=>$value){
				$imgnew = $img->down_load($value['vod_miaopic'],'vod');
				if($value['vod_miaopic'] == $imgnew){
					$rs->where('vod_id='.$value['vod_id'])->setField('vod_miaopic',str_replace("http://","httpf://",$value['vod_miaopic']));
					echo(($key+1).' <a href="'.$value['vod_miaopic'].'" target="_blank">'.$value['vod_miaopic'].'</a> <font color=red>下载失败!</font><br/>');
				}else{
					$rs->where('vod_id = '.$value['vod_id'])->setField('vod_miaopic',$imgnew);
					echo(($key+1).' <a href="'.$value['vod_miaopic'].'" target="_blank">'.$value['vod_miaopic'].'</a> 下载成功！<br />');					
				}
				ob_flush();flush();
			}
			echo'请稍等一会，正在释放服务器资源...<meta http-equiv="refresh" content='.C('play_collect_time').';url=?s=Admin-Picc-Down>';
			echo'</div>';
		}else{
			$count = $rs->where('Left(vod_miaopic,8)="httpf://"')->count('vod_id');
			if($count){
				echo'<div style="font-size:14px;">共有<span>'.$count.'</span>张远程图片保存失败,如果需要重新下载,请点击<a href="?s=Admin-Picc-Down-id-fail">[这里]</a>!</div>';
			}else{
	...
	public function ajaxpic(){
		...
		if(stristr($path,'/vod')){
			$rs = M("Vod");
			$array = $rs->field('vod_miaopic')->where('Left(vod_miaopic,4)!="http"')->order('vod_addtime desc')->select();
			foreach ($array as $value){
				$dir2[] = $value['vod_miaopic'];
			}			
