<?php
class PicsuoAction extends BaseAction{
	//下载远程图片
    public function down(){
		...
		if('fail'==trim($_GET['id'])){
			$rs->execute('update '.C('db_prefix').'vod set vod_miaosuopic=REPLACE(vod_miaosuopic,"httpf://", "http://")');
		}
		$count = $rs->where('Left(vod_miaosuopic,7)="http://"')->count('vod_id');
		$list = $rs->where('Left(vod_miaosuopic,7)="http://"')->order('vod_addtime desc')->limit(C('upload_http_down'))->select();
		if($list){
			...
			foreach($list as $key=>$value){
				$imgnew = $img->down_load($value['vod_miaosuopic'],'vod');
				if($value['vod_miaosuopic'] == $imgnew){
					$rs->where('vod_id='.$value['vod_id'])->setField('vod_miaosuopic',str_replace("http://","httpf://",$value['vod_miaosuopic']));
					echo(($key+1).' <a href="'.$value['vod_miaosuopic'].'" target="_blank">'.$value['vod_miaosuopic'].'</a> <font color=red>下载失败!</font><br/>');
				}else{
					$rs->where('vod_id = '.$value['vod_id'])->setField('vod_miaosuopic',$imgnew);
					echo(($key+1).' <a href="'.$value['vod_miaosuopic'].'" target="_blank">'.$value['vod_miaosuopic'].'</a> 下载成功！<br />');					
				}
				ob_flush();flush();
			}
			echo'请稍等一会，正在释放服务器资源...<meta http-equiv="refresh" content='.C('play_collect_time').';url=?s=Admin-Picsuo-Down>';
			echo'</div>';
		}else{
			$count = $rs->where('Left(vod_miaosuopic,8)="httpf://"')->count('vod_id');
			if($count){
				echo'<div style="font-size:14px;">共有<span>'.$count.'</span>张远程图片保存失败,如果需要重新下载,请点击<a href="?s=Admin-Picsuo-Down-id-fail">[这里]</a>!</div>';
			}else{
	...
	public function ajaxpic(){
		...
		if(stristr($path,'/vod')){
			$rs = M("Vod");
			$array = $rs->field('vod_miaosuopic')->where('Left(vod_miaosuopic,4)!="http"')->order('vod_addtime desc')->select();
			foreach ($array as $value){
				$dir2[] = $value['vod_miaosuopic'];
			}			
