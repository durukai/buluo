<?php
class XmlnapianzyAction extends BaseAction
{
    // 资源列表
    ...
    public function setbind()
    {
        ...
        $this->assign('vod_cid', $array_bind[$_GET['bind']]);
        //绑定后的系统分类
    ...
    public function _empty()
    {
        ...
        if (in_array(ACTION_NAME, $array)) {
            $this->xml_httpurl(ACTION_NAME);
        } else {
            $this->error('不支持的资源站接口。');
        }
    }
    ...
    public function xml_httpurl($actionname)
    {
        ...
        $array_url = array();
        //本地程序跳转参数
        $xml_function_name = 'xml_' . $actionname;
        //不同资源库程序调用不同的处理函数
        //获取跳转参数
        $array_url['action'] = $_GET['action'];
        //是否入库(all/day/ids)
        $array_url['fid'] = intval($_GET['fid']);
        //合作渠道ID
        $array_url['xmlurl'] = $_GET['xmlurl'];
        //资源库网址
        $array_url['reurl'] = $_GET['reurl'];
        //来源网址
        $array_url['pic'] = $_GET['pic'];
        //重采资料
        //
        $array_url['vodids'] = $_GET['vodids'];
        if ($_POST['ids']) {
            $array_url['vodids'] = implode(',', $_POST['ids']);
        }
        //指定视频ID
        $array_url['cid'] = $_GET['cid'];
        //指定视频分类
        ...
        //指定关键字
        $array_url['h'] = intval($_GET['h']);
        //指定时间
        $array_url['p'] = !empty($_GET['p']) ? intval($_GET['p']) : 1;
        $array_url['page'] = $array_url['p'];
        //指定分页
        //
        $array_url['inputer'] = $_GET['inputer'];
        //指定资源库频道
        $array_url['play'] = $_GET['play'];
        //指定播放器组(如不指定则为目标站的全部播放器组)
        // 调用抓取数据处理函数
    ...
    public function xml_feifei($array_url)
    {
        ...
        if ($json) {
            ...
            $array_tpl = array();
            $array_url['p'] = '{!page!}';
            $array_tpl['pageurl'] = U('Admin-Xmlnapianzy/Feifei', $array_url);
            ...
            foreach ($json->data as $key => $value) {
				...
				$array_vod[$key]['vod_letters'] = $this->getRandChar(20);
            }
            //dump($array_vod);
    ...
    {
        //组合资源库URL地址并获取XML资源
		@require('./napianzy/token.php');
        $array_tpl['httpurl'] = '&wd=' . urldecode($array_url['wd']) . '&t=' . $array_url['cid'] . '&h=' . $array_url['h'] . '&ids=' . $array_url['vodids'] . '&pg=' . $array_url['page'].'&url='.$_SERVER['HTTP_HOST'].'&token='.$USER_TOKEN.'';
        if ($array_url['action']) {
        ...
        if ($xml) {
            ...
            $array_url['p'] = '{!page!}';
            $array_tpl['pageurl'] = U('Admin-Xmlnapianzy/Caijia', $array_url);
            ...
            foreach ($vod_array[1] as $key => $value) {
                ...
                $vod_array[17][$key] = str_replace(' ', ',', $vod_array[17][$key]);
                $vod_array[16][$key] = str_replace(' ', ',', $vod_array[16][$key]);
                $listvod[$key]['vod_keywords'] = $vod_array[17][$key] . ',' . $vod_array[16][$key];
                //关联词
                $listvod[$key]['vod_gold'] = intval($vod_array[18][$key]);
                //评分值
                $listvod[$key]['vod_golder'] = intval($vod_array[19][$key]);
                //评分人数
                $listvod[$key]['vod_stars'] = intval($vod_array[20][$key]);
                //推荐
                if ($vod_array[20][$key] > 0) {
                    $listvod[$key]['vod_stars'] = 5;
                }
                //指定5星
                $listvod[$key]['vod_inputer'] = $array_url['fid'] . '_' . $listvod[$key]['vod_id'];
                ...
                if ($array_url['action']) {
                    ...
                    $listvod[$key]['vod_play'] = str_replace(array('泥巴影音', '乐视', '奇艺', '土豆', 'qq播客', '搜狐', '百度影音', 'youku', '56', '优酷', '迅播高清', 'SWF数据'), array('niba', 'letv', 'qiyi', 'tudou', 'qq', 'sohu', 'bdhd', 'yuku', 'wole', 'yuku', 'gvod', 'swf'), implode('$$$', $url_arr[1]));
    ...
    {
        $arr_tmp = explode('$$$', $playurl);
        $arr_res = array();
        foreach ($arr_tmp as $one) {
            $arr_ji = explode('#', str_replace('||', '//', $one));
            $array_url = array();
            ...
            $array_res[] = implode(chr(13), $array_url);
        }
        return implode('$$$', $array_res);
    }
}