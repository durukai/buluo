<?php
class XmlfangaoAction extends BaseAction
{
    // 资源列表
    ...
    public function xml_feifei($array_url)
    {
        ...
        if ($json) {
            ...
            $array_tpl = array();
            $array_url['p'] = '{!page!}';
            $array_tpl['pageurl'] = U('Admin-Xmlfangao/Feifei', $array_url);
    ...
    {
        //组合资源库URL地址并获取XML资源
        $array_tpl['httpurl'] = '&wd=' . urldecode($array_url['wd']) . '&t=' . $array_url['cid'] . '&h=' . $array_url['h'] . '&ids=' . $array_url['vodids'] . '&pg=' . $array_url['page'].'&url='.$_SERVER['HTTP_HOST'];
        if ($array_url['action']) {
        ...
        if ($xml) {
            ...
            $array_url['p'] = '{!page!}';
            $array_tpl['pageurl'] = U('Admin-Xmlfangao/Caijia', $array_url);
