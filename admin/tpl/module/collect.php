...
{
	$plt->set_file('main', $ac.'_'.$method.'.html');
	$accesskey = npcjen($npcjarr,$USER_TOKEN).'&acc='.npcjen($_SERVER['HTTP_HOST'],'').'&key='.npcjen($USER_TOKEN,'');
    ...
    elseif($xt=='1'){
    	$url = $apiurl . '?ac=list&pg=' . $pg . '&rid='.$group . '&t=' . $type . '&wd=' . urlencode($wd).'&sec='.$accesskey;
		...
		case 'sel':
			...
			switch($xt)
			{
				case '0': $url = "?action=cjsel&ids=".$ids;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&ids=".$ids.'&sec='.$accesskey;
					break;
		...
		case 'day':
			switch($xt)
			{
				case '0': $url = "?action=cjday&rday=".$hour."&rpage=".$pg;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&h=".$hour."&pg=".$pg.'&sec='.$accesskey;
					break;
		...
		case 'type':
			...
			switch($xt)
			{
				case '0': $url = "?action=cjtype&rpage=".$pg."&rtype=". $type;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&pg=" . $pg . "&t=" . $type.'&sec='.$accesskey;
					break;
		...
		case 'all':
			switch($xt)
			{
				case '0': $url = "?action=cjall&rpage=".$pg;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&pg=". $pg.'&sec='.$accesskey;
					break;
    ...
    foreach($array3[1] as $key=>$value){
        ...
        elseif(strpos(','.$filter,$d_name)) { $des = '<font color="red">数据在过滤单中，跳过。</font>'; }
	        ...
	        if(!$row){
		    	...
		    	$cols = array("d_type", "d_name","d_enname","d_letter","d_state","d_remarks","d_tag","d_pic",'d_picthumb',"d_hits","d_dayhits","d_weekhits","d_monthhits","d_score","d_scoreall","d_scorenum","d_starring", "d_directed","d_year","d_area","d_lang","d_addtime","d_time","d_hide","d_content");
		    	$vals = array($d_type,$d_name,$d_enname,$d_letter,$d_state,$d_remarks,$d_tag,$d_pic,$d_picthumb,$d_hits,$d_dayhits,$d_weekhits,$d_monthhits,$d_score,$d_scoreall,$d_scorenum,$d_starring,$d_directed,$d_year,$d_area, $d_lang,$d_addtime,$d_time,$d_hide,$d_content);
...
function npcjen($string = '', $skey = 'nepian') {
