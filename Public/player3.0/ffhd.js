...
function $PlayerNt(){
	if (navigator.plugins) {
		...
		if(!install){
			player = '<object id="FFHDPlayer" name="FFHDPlayer" type="application/npFFPlayer" width="100%" height="'+Player.Height+'" progid="XLIB.FFPlayer.1" url="'+Player.Url+'" CurWebPage="'+ffhd_weburl+'"></object>';
			return player;
		}
	}
	Player.Install();
...
function $PlayerIe(){
	player = '<object classid="clsid:D154C77B-73C3-4096-ABC4-4AFAE87AB206" width="100%" height="'+Player.Height+'" id="FFHDPlayer" name="FFHDPlayer" onerror="Player.Install();"><param name="url" value="'+Player.Url+'"/><param name="CurWebPage" value="'+ffhd_weburl+'"/></object>';
	return player;
}
//beta7版播放器回调函数
var onPlay = function(){
