...
function $BdhdUrl(url,urlname){
	if(url.indexOf("dhd://")>0){
		...
		return(onurl[0]+"|"+onurl[1]+"|"+Player.VodName+urlname+"[www.feifeicms.com]."+suffix);
	}
	return(url);
}
function $PlayerNt(){
...
function $PlayerIe(){
	player = '<object classid="clsid:02E2D748-67F8-48B4-8AB4-0A085374BB99" width="100%" height="'+Player.Height+'" id="BaiduPlayer" name="BaiduPlayer" onerror="Player.Install();" style="display:none"><param name="URL" value="'+Player.Url+'"/><param name="NextCacheUrl" value="'+Player.NextUrl+'"><param name="LastWebPage" value="'+Player.LastWebPage+'"><param name="NextWebPage" value="'+Player.NextWebPage+'"><param name="OnPlay" value="onPlay"/><param name="OnPause" value="onPause"/><param name="OnFirstBufferingStart" value="onFirstBufferingStart"/><param name="OnFirstBufferingEnd" value="onFirstBufferingEnd"/><param name="OnPlayBufferingStart" value="onPlayBufferingStart"/><param name="OnPlayBufferingEnd" value="onPlayBufferingEnd"/><param name="OnComplete" value="onComplete"/><param name="Autoplay" value="1"/></object>';
	return player;
}
//beta7版播放器回调函数
var onPlay = function(){
