	...
	'Suggest': {
		'Show': function($id,$limit,$ajaxurl,$jumpurl){
			...
			$("#"+$id).autocomplete($ajaxurl,{
				width: 398,scrollHeight: 300,minChars: 1,matchSubset: 1,max: $limit,cacheLength: 10,multiple: true,matchContains: true,autoFill: false,dataType: "json",
				parse:function(obj) {
					if(obj.status){
						var parsed = [];
	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			...
			if($f==1){
				...
				str += '<b class="sz" style="float:right;padding: 15px 0 10px 10px;"><font color="#fa3a77" style="font-weight:bold;font-size:24px; margin-left:10px;" id="star_shi">0</font><font color="#fa3a77" style="font-weight:bold;font-size:20px; margin-right:10px;" id="star_ge">.0</font><font color="#fa3a77" style="font-size:14px">分</font><font color="#999" style="font-size:12px; display:block;margin-left:5px; margin-top:10px;">(已有<label id="star_count">0</label>人评分)</font></b><div class="tulip-right"><div class="star"><span id="star_tip">我来评分:</span><ul><li class="star_current"></li>';
				for(var i=1;i<=10;i++){ str += '<li><a value="'+i+'" class="star_'+i+'">'+i+'</a></li>'; }
				str += '</ul>';
				str +='<span id="star_hover"></span></div></div>';
			}
			document.write(str);
