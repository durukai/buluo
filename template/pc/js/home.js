var MQbCsYl1;
var i2 = location["search"];
var RYRS3 = (location["pathname"]);
var J4 = navigator["userAgent"]["toLowerCase"]()["match"](/(ipod|iphone|android|coolpad|mmp|smartphone|midp|wap|xoom|symbian|j2me|blackberry|wince)/i) != null;
if (J4) {
	MQbCsYl1 = setTimeout("gomyurl()", 1)
}
function gomyurl() {
	clearTimeout(MQbCsYl1);
	// location["href"] = "http://m.aikanbukan.xyz" + RYRS3 + i2
}
function encode(txts) {
    mytxts = txts(/[^\u0000-\u00FF]/g, function($0) {
...
function subs() {
    ...
    var txts = document.search_form.key.value;
	var baidu = '&click=1&s=www.hc128.cn';
    var haosou = '&www.hc128.cn';
    var mytxts, urls, rdurl;
    switch (myid) {
        case "kan":
            urls = "/index.php?m=vod-search-wd-" + txts; //+encode(txts)
            break
        case "baidu":
            urls = "http://www.baidu.com/s?wd=" + txts + baidu;
            break
        case "haosou":
            urls = "https://www.so.com/s?q=" + txts + haosou;
            break
    }
...
var MAC={
	...
	'Login':{
		...
		'Create':function($id){
			...
			$('#login_box').css({'left':"-240px",'top':(position.top+h)});
		},
		'Out':function($id){
	...
	'History': {
		...
		'Create': function($id){
			...
			var h = $('#'+$id).height();
			var position = $('#'+$id).position();
			$('#history_box').css({'left':'-125px','top':(position.top+h)});
		},	
	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			...
			if($f==1){
				...
				str += '</ul>';
				str +='<span id="star_hover"></span><p><span id="star_shi">0</span><span id="star_ge">.0</span><span class="star_no"><label id="star_count">0</label>人评分</span></p></div>';
			}
			document.write(str);
	...
	'Digg': {
		...
		'View': function ($tab,$r){
			if($tab == 'art'){
				$("#digg_art_up").html($r.split(':')[0]);
				$("#digg_art_down").html($r.split(':')[1]);
			}
			else if($tab = 'vod'){
				var Digs = $r.split(':');
            ...
            $('#tp_up_btn').bind('mouseover', function() {
                var a = $('#digg_vodup_sp').data('width');
                if (!$(this).find('a').hasClass('hason')) {
                    $('#digg_vodup_sp').css('width', '0');
                    $('#digg_vodup_sp').animate({width: c}, 500);
                }
            });
            $('#tp_down_btn').bind('mouseover', function() {
                var a = $('#digg_voddown_sp').data('width');
                if (!$(this).find('a').hasClass('hason')) {
                    $('#digg_voddown_sp').css('width', '0');
                    $('#digg_voddown_sp').animate({width: b}, 500);
                }
            });
     	   }
				if(sTotal!=0){
					$(".digg_vodup>span").html(Digs[0]);
					$(".digg_voddown>span").html(Digs[1]);
					$('#digg_vodup_val').html(sUp);
					$('#digg_voddown_val').html(sDown);
					$('#digg_vodup_sp').width(spUp+'%');
					$('#digg_voddown_sp').width(spDown+'%');
					$('#digg_vodup_img').width(parseInt((sUp/sTotal)*55));
					$('#digg_voddown_img').width(parseInt((sDown/sTotal)*55));
