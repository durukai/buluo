...
var MAC={
	...
	'Score': {
		...
		'Show':function($f,$id){
			...
			document.write(str);
			$.ajax({type: 'get',url: SitePath + this.ajaxurl + '&id='+$id,timeout: 5000,
				error: function(){
					$(".star").html('评分加载失败');
				},
				success: function($r){
					...
					if($f==1){
						...
						$(".star>ul>li>a").click(function(){
							MAC.Score.Send( '&id='+$id+'&score='+$(this).attr('value') );
						});
					}
				}
			});
		...
		'View':function($r){
			$r = eval('(' + $r + ')');
			$r.pjf = parseFloat($r.pjf).toFixed(1);
			$("#star_count").html( $r.count );
			$("#star_all").html( $r.score );
			$("#star_pjf").html( $r.pjf );
			$("#star_shi").html( parseInt($r.pjf) );
			$("#star_ge").html( "." +  $r.pjf.toString().split('.')[1] );
			$(".star_current").width($r.pjf*10);
		}
	},
	'Digg': {
