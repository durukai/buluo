...
function PlayHistoryClass() {
                ...
                if (result["rcode"] > -1) {
                    ...
                    if (result["r"] != null && result["r"].length > 0) {
                        ...
                        for (var i in result["r"]) {
                            var data = result["r"][i];
                            innerStr += '<li><h5><a href="' + data["vod_readurl"] + '">' + data["vod_name"] + "</a><em>/</em><a href='" + data["vod_palyurl"] + "' target='_blank'>" + data["url_name"] + '</a></h5><label><a class="color" href="' + data["vod_palyurl"] + "\">继续观看</a></label><a href=\"javascript:;\" target='_blank' class='delck' data=\"" + data["id"] + "\" mtype='ajax'>删除</a></li>";
                        }
            ...
            if (nameArray != undefined && nameArray != null) {
                for (var i = nameArray.length - 1; i >= 0; i--) {
                    ...
                    if (vodnames.length == 2 && vodlinks.length == 2) {
                        innerStr += '<li><h5><a href="' + vodnames[1] + '">' + vodnames[0] + "</a><em>/</em><a target='_blank' href=\"" + vodlinks[1] + '">' + vodlinks[0] + '</a></h5><label><a class="color" target=\'_blank\' href="' + vodlinks[1] + "\">继续观看</a></label><a href=\"javascript:;\" class='delck' data='" + i + "' mtype='inck'>删除</a></li>";
                    }
                }
            }
            $("#his-todo").show();
    ...
    this.emptyhistory = function(type) {
        $.showfloatdiv({
            ...
            suredo: function(e) {
                if (type == "ajax") {
                    $.get(Root + "index.php?s=User-Comm-emptyhistory", function(r) {
                        document.getElementById("playhistory").innerHTML = "<li class='no-his'><p>暂无播放历史列表...</p></li>";
                    });
                } else {
                    _GC();
...
function qrsearch(id) {
    if ($("#xdwwd").val() == "请在此处输入影片片名或演员名称。" || $("#wd").val() == "") {
        ...
        document.location = Root + "index.php?s=vod-search-wd-" + encodeURIComponent($("#wd").val()) + ".html";
    }
    return false;
}
            ...
            $self.one("appear", function() {
                if (!this.loaded) {
                    ...
                    $("<img />").bind("load", function() {
                        ...
                        $self.attr("src", settings.errorimgpic);
...
$(document).ready(function() {
    ...
    if ($("#actorall").length > 0) {
        if ($("#actorall")[0].scrollHeight > 200) {
            $("#downzk").show();
            $("#actorall").height(200);
            $("#downzk").click(function(e) {
                if ($(this).hasClass("ss")) {
                    ...
                    $(this).removeClass("zk").addClass("ss").text("收起部分");
                }
                if ($("#actorall").height() > 200) {
    ...
    a.fn.slide = function(b) {
        return a.fn.slide.defaults = {
            ...
            if (j.children().each(function() {
                a(this).width() > z && (z = a(this).width(), y = a(this).outerWidth(!0)), a(this).height() > A && (A = a(this).height(), 
            ...
            var G = function() {
                ...
                if (D != n || a || "leftMarquee" == d || "topMarquee" == d) {
                    ...
                    h.removeClass(c.titOnClassName).eq(n).addClass(c.titOnClassName), D = n, 0 == w && (f.removeClass("nextStop"), 
                    e.removeClass("prevStop"), 0 == n ? e.addClass("prevStop") : n == i - 1 && f.addClass("nextStop")), 
                    g.html("<span>" + (n + 1) + "</span>/" + i);
                }
            };
            J(!0), t && ("leftMarquee" == d || "topMarquee" == d ? (u ? n-- : n++, C = setInterval(J, p), 
            j.hover(function() {
                t && clearInterval(C);
            }, function() {
